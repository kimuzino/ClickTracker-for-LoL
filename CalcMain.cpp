...
// Update mechanic
// Checks for update from github somehow, like search from cpp file for something like // Version:1.0
// After the new exe is downloaded it deletes the old one
// Copies all the old files information and pastes them to the new fresh files
// New fresh exe/app should be working
// Make sure it doesnt repeat and update checking works

// Make another .cpp for this (Going to be too big)


// This is maybe done
// Previos game comparison, see how you improved or got worse. Possibility to see the result (not shown automatically) by pressing key (keycode)Pause for now. Change to pg up


// Pause and resume. make a file that saves all the progress and next launch it displays the file and deletes it (Keycode)
// File: Contains All the previous games stats, next game, and previous games stats for comparison. 
// Moves the values to the file and closes the cmd. 
// next launch displays all previous games, saves previous stats to the map, changes (int rounds = 1;) to the ammount of games played; 


// Make changes
// Delete see stats and move it to previous stats // Change name (Stats preview) (????)
...
{
    string color = "red";
    string weight = "2000kg";
    int maxSpeed = 200;
}

car MyCar = new Car(); /Request cars color from the container
console.writeline(MyCar.color); / Class.variable
console.writeline(MyCar.weight);
